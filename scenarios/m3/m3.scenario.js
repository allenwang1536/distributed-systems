...
*/
  ...
  distribution.local.groups.put(config, groupB, (e, v) => {
    ...
    distribution.local.groups.add('groupB', n3, (e2) => {
      distribution.groupB.status.get('nid', (e3, v) => {
        try {
  ...
  distribution.local.groups.put(config, groupC, (e, v) => {
    distribution.groupC.groups.put(config, groupC, (e, v) => {
      ...
      distribution.local.comm.send(
          [{gid: 'groupC'}, {[id.getSID(n2)]: n2}],
          {node: n1, gid: 'local', service: 'groups', method: 'put'},
          (e, v) => {
...
*/
  ...
  const config = {gid: 'groupD', subset: (lst) => 1};
